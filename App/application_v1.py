...
@st.cache
def setup():
	dataset=pd.read_csv('admis.csv')
	dataset.columns=['trace','admissible','matrix','decomp','length','final_position']
	dataset['commutator']= pd.get_dummies(dataset['final_position'])['{0, 0}']
	#change the form of the decomp column and the matrices to work better with python
	replacement_column=[]
	matrix_replacement_column=[]
	for i in range(0, len(dataset)):
		temp=ast.literal_eval(dataset.decomp.values[i].replace("{", "[").replace("}", "]"))
        #when i change to the new file this needs to become a 0 and a 1 but for now it isfine
		replacement_column.append([temp[1],temp[2]])
		matrix_replacement_column.append(ast.literal_eval(dataset.matrix.values[i].replace("{", "[").replace("}", "]")))
	dataset=pd.DataFrame({'trace': dataset.trace, 'matrix': matrix_replacement_column, 'decomp': replacement_column,"commutator": dataset.commutator})
...
@st.cache
def setup2():
	genusinfo=pd.read_csv('results.csv')
	list1=[]
	list2=[]
	#finding the class number and the number of commutator subgroups
	for t in genusinfo.trace.values:
		all_of_it=dataset.loc[dataset.trace==t].commutator
		list1.append(len(all_of_it))
		list2.append(sum(all_of_it))
	return pd.DataFrame({"trace":genusinfo.trace, "L_class_number": list1,"commutator_class_number": list2, "genus":genusinfo.genus})
...
def matching_pairs(asbs, sizes):
	...
	for cval in thec:
		for dval in thed:
			...
			minigenus=makeTheMatrix(makeTheGraph(thispos,sizes),sizes)
...
def genusfinder(matrix_rep):
	...
	allPossibleMatches=matching_pairs(convertToAandB(matrix_rep), sizes)     
...
if tell_me=="What does admissible mean?":
	...
	st.write("Given that $\\Gamma'(2)$ is a subgroup of $L$, we only look at equivlency classes of matrices that lie inside $L$ and then ask the additional question of if they lie in $\\Gamma'(2)$.  Thus for a given $t$, $h(t)$ is the number of conjugacy classes of matrices with trace $t$ that lie in $L$ and $h'(t)$ is the number of such matrices that also lie in $\\Gamma'(2)$.")
elif tell_me=="Why are some in blue?":
	st.write("The matrices and decompositions in blue just mean that they themselves are in the commutator subgroup $\\Gamma'(2)$.")    
...
if decom:
	...
	for i in range(0, len(allmats)):
		m=allmats[i][0]
		if allmats[i][1]==0:
			st.write(str(i+1)+": $"+ str(m) +" \\hspace{5pt}$ ")
		else:
			st.write(str(i+1)+":  $\\textcolor{blue}{"+ str(m) +"} \\hspace{5pt}$ ")
##########################################################################################################
				...
				if len(lists[0])==len(newlist(lists[0])) and len(lists[1])==len(newlist(lists[1])): 
					...
					if genusq: 
						if len(lists[0])<=3:
							...
							if sum([abs(ele) for ele in lists[0]])+sum([abs(ele) for ele in lists[1]])<18:
								...
								ThetaArray = np.linspace(0, 2*np.pi, len(results[1]), endpoint=False)
								x = np.cos(ThetaArray)
								y = np.sin(ThetaArray)
